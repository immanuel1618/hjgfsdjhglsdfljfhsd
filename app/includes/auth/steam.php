...
if ( ! empty( $_GET["auth"] ) && $_GET["auth"] == 'login' ) {
    ...
    try 
    {
        ...
        if ( ! $openid->mode ) 
        {
            ...
            if ( ! headers_sent() ) {?>
                <script type="text/javascript">window.location.href="<?= $openid->authUrl() ?>";</script>
                <noscript><meta http-equiv="refresh" content="0;url=<?= $openid->authUrl() ?>" /></noscript>
                <?php exit;
            }
        } 
...
if ( ! empty( $_GET["auth"] ) && $_GET["auth"] == 'logout' ) 
{
    ...
    header('Location: ' . $this->General->arr_general['site']);
    if ( ! headers_sent() ) {?>
        <script type="text/javascript">window.location.href="<?= $this->General->arr_general['site'] ?>";</script>
        <noscript><meta http-equiv="refresh" content="0;url=<?= $this->General->arr_general['site'] ?>" /></noscript>
        <?php exit;
    }
}
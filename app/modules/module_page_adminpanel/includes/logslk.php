<div class="col-md-12">
	<select style="display: none;" class="custom-select" onchange="window.location.href=this.value" placeholder="<?= $Translate->get_translate_module_phrase('module_page_adminpanel', '_Logs_pay'); ?>">
			...
			<div class="lk_logs_wrap scroll no-scrollbar">
				<?php foreach ($Admin->LkLogs() as $log) : ?>
					<div style="display: flex; justify-content: space-between; gap: 3px">
						<a href="<?= set_url_section(get_url(2), 'log', $log['log_name']) ?>">
							<?= $log['log_name'] ?>
						</a>
						<button id="log_del_lk" id_del="<?= $log['log_name'] ?>">
		...
		<div class="card">
			<div class="card-header">
				<h5 class="badge"><?= $Translate->get_translate_module_phrase('module_page_pay', '_LogContent'); ?>
					<div class="copy_log" data-clipboard-text="<?php foreach ($Admin->LkLogContent($_GET['log']) as $key) : ?><?= $key['log_name'] . $key['log_time'] . LangValReplace($Translate->get_translate_module_phrase('module_page_pay', $key['log_content']), json_decode(str_replace('[]', '', $key['log_value']), true)); ?><?php endforeach; ?>">
			...
			<div class="card-container no-scrollbar" style="max-height: 48vh; overflow: hidden; overflow-y: scroll;">
				...
				<?php foreach ($Admin->LkLogContent($_GET['log']) as $key) : ?>
					<div class="log_text"><?= $key['log_name'] . $key['log_time'] . LangValReplace($Translate->get_translate_module_phrase('module_page_pay', $key['log_content']), json_decode(str_replace('[]', '', $key['log_value']), true)); ?></div>
				<?php endforeach; ?>
			</div>
		</div>
	</div>

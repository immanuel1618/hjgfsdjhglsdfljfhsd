    ...
    switch ($res_system[$server_group]['admin_mod']) {
        case 'AdminSystem':
            ...
            if ($server_id != 'all') {
                ...
                $MSBan = $Db->queryAll('AdminSystem', 0, 0, "SELECT * FROM `as_punishments` WHERE `punish_type`= 0 AND (`server_id` = '$server_id' OR `server_id` = '-1') ORDER BY `created` DESC LIMIT {$page_num_min}," . PLAYERS_ON_PAGE . "");
                $MSMute = $Db->queryAll('AdminSystem', 0, 0, "SELECT * FROM `as_punishments` WHERE `punish_type` != 0 AND (`server_id` = '$server_id' OR `server_id` = '-1')  ORDER BY `created` DESC LIMIT {$page_num_min}," . PLAYERS_ON_PAGE . "");
                ...
                ORDER BY
                    `group_id` ASC");
                $MSAdmin = array_values(array_slice($MSAdminArray, $page_num_min, PLAYERS_ON_PAGE));
                $MSBan = $Db->queryAll('AdminSystem', 0, 0, "SELECT * FROM `as_punishments` WHERE `punish_type`= 0 ORDER BY `created` DESC LIMIT {$page_num_min}," . PLAYERS_ON_PAGE . "");
                $MSMute = $Db->queryAll('AdminSystem', 0, 0, "SELECT * FROM `as_punishments` WHERE `punish_type` != 0 ORDER BY `created` DESC LIMIT {$page_num_min}," . PLAYERS_ON_PAGE . "");

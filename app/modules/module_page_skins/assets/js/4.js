...
let pagestickers = 1;
let selectedPlayerCollections = false;
let collectionsMorePage = 0;
...
$(document).ready(function () {
    ...
    function getCollectionID(url) {
        const regex = /\/collections\/(\d+)\//; // Регулярное выражение для поиска ID коллекции
        const match = url.match(regex); // Ищем совпадение в URL

        if (match && match[1]) { // Если совпадение найдено
            return match[1]; // Возвращаем ID коллекции
        } else {
            return null; // Если не найдено, возвращаем null
        }
    }
    let collectionID = getCollectionID(url);
    ...
    $('#search_js_skins').on('input', function() {
        ...
        if(page_php == "collections") { interfaceCollections(searchText); } 
    });
    ...
    $(document).on('click', '.choosing_sides', function () {
        ...
        if(page_php == "collections") { interfaceCollections(); }
    });
    ...
    $(document).on('click', '.collections__items-list-more', function () {
        collectionsMorePage += 1;
        interfaceCollections('', selectedPlayerCollections, collectionsMorePage);
    });
    
    function interfaceCollections(searchtext = '', playerCollections = false, more = 0) {
        let requestID = playerCollections ? { PlayerCollections: true, search_text: searchtext, more: more } : { html_collections: true, search_text: searchtext, more: more };
        if (searchtext.trim() == '' && more == 0) {
            html_js.css('display', 'none');
            showLoader();
            collectionsMorePage = 0;
        }
        $.ajax({
            type: 'post',
            url: location.href,
            ...
            success: function (data) {
                if (searchtext.trim() != '' || more > 0) {
                    const collectionsList = document.querySelector('.collections__items-list');
                    if (more > 0) {
                        collectionsList.innerHTML += data['collections_html'];
                    } else {
                        collectionsList.innerHTML = data['collections_html'];
                    }
    
                    const moreButton = document.querySelector('.collections__items-list-more');
                    if (data['moreCheck'] == true && collectionsList.children.length >= 4) {
                    ...
                    hideLoader();
                    html_js.html(data.html).css('display', 'block');
                }
                loaderIMG(1);
            },
            error: function (error) {
    ...
    const test = () => {
        document.addEventListener('click', (e) => {
            const collectionCard = e.target.closest('.collections__items-list-collection');
            const backButton = e.target.closest('.collections__items-search-back');
            const weaponShow = e.target.closest('.collections__items-details-body-list-show');
            const hiddenItemsContainer = e.target.closest('.collections__items-details-body-list');
            const hiddenItems = hiddenItemsContainer ? hiddenItemsContainer.querySelectorAll('.collections__items-details-body-list-item:nth-child(n+12)') : null;
            const check = e.target.closest('.collections__items-search-info-params-item-check');

            if (collectionCard) {
                weaponsHidden = true;
                collectionID = collectionCard.getAttribute('data-id');
                if (!collectionID) {
                    console.error('collectionID не найден');
                    return;
                }
                const url = `/skins/collections/${collectionID}/`;
                history.pushState({ collectionID }, null, url);
                interfaceDetailsCollection(collectionID, activeSide, '', weaponsHidden);
            }
    
            if (backButton) {
                const url = `/skins/collections/`;
                history.pushState('collections', null, url);
                interfaceCollections();
                collectionID = false;
            }
            ...
            if (weaponShow) {
                weaponsHidden = false;
                weaponShow.parentElement.classList.remove('hidden');
                weaponShow.classList.add('hidden');
            ...
            if (check) {
                const param = check.parentElement.getAttribute('data-type');
                const isCurrentlyActive = check.classList.contains('active');
            
                if (param === 'MyCollection' && !isCurrentlyActive) {
                    selectedPlayerCollections = true;
                    collectionsMorePage = 0;
                    $.ajax({
                        type: 'post',
                        url: location.href,
                        data: { PlayerCollections: true, isActive: 1 },
                        dataType: 'json',
                        global: false,
                        success: function (data) {
                            hideLoader();
                ...
                } else if (param === 'MyCollection' && isCurrentlyActive) {
                    selectedPlayerCollections = false;
                    collectionsMorePage = 0;
                    $.ajax({
                        type: 'post',
                        url: location.href,
                        data: { html_collections: true, isActive: 0 },
                        dataType: 'json',
                        global: false,
                        success: function (data) {
                            hideLoader();
    ...
    function interfaceDetailsCollection(collectionID) {
        console.log(`Запрос на коллекцию с ID: ${collectionID}`);
        html_js.css('display', 'none');
        showLoader();
        collectionsMorePage = 0;
        $.ajax({
            type: 'post',
            url: location.href,
            data: { html_collectiondeteils: true, collectionID: collectionID },
            dataType: 'json',
            global: false,
            success: function (data) {
                if (data && data.html) {
                    ...
                    hideLoader();
                    html_js.html('Ошибка загрузки коллекции');
                }
            },
            error: function (error) {
                console.log(error);
                hideLoader();
                html_js.html('Ошибка JS - 404...');
            }
        });
    }
    ...
    if (currentUrl.startsWith("/skins/collections/")) {
        const collectionID = currentUrl.split('/')[3];
        if (collectionID) {
            console.log(1)
            interfaceDetailsCollection(collectionID);
        } else {
            interfaceCollections();
        }
    } else if (page_php === "collections") {
        interfaceCollections();
    }
    
    $(document).on('click', '#CollectionModalWeapon', function () {
        const idName = $(this).attr('data-name');

        if ($(this).data('name') === 'Knife') {
            interfaceWeaponsKnifes();
            $('.skin-modal, .skin-modal-overlay').addClass('active-modal');
        } else if 
            ($(this).data('name') === 'Gloves') {
            interfaceWeaponsGloves();
            $('.skin-modal, .skin-modal-overlay').addClass('active-modal');
        }
        else {
            modalCollectionSkins(idName);
            $('.skin-modal, .skin-modal-overlay').addClass('active-modal');
        }
    });


    function modalCollectionSkins(idname) {
        $.ajax({
            type: 'post',
            url: location.href,
            data: { id_namessss: idname, collectionID: collectionID},
            dataType: 'json',
            global: false,
            success: function (data) {
                $('#skin-modal-js').html(data.html);
                ...
                const activeChoice = $('.choice_active');
                if (activeChoice.length) {
                    const skinNameElement = activeChoice.find('.data-name');
                    const skinNameText = skinNameElement.length ? skinNameElement.text() : '-';
                    my_skin.text(skinNameText);
                } else {
                    my_skin.text('-');
    ...
    function interfaceWeaponsGloves() {

        $.ajax({
            type: 'post',
            url: location.href,
            data: { id_name_gloves: true, collectionID: collectionID},
            dataType: 'json',
            global: false,
            success: function (data) {
                $('#skin-modal-js').html(data.html);
    ...
    function interfaceWeaponsKnifes() {

        $.ajax({
            type: 'post',
            url: location.href,
            data: { id_name_knife: true, collectionID: collectionID},
            dataType: 'json',
            global: false,
            success: function (data) {
                $('#skin-modal-js').html(data.html);
    ...
    $(document).on('click', '#CollectionSkinChangerUpdate', function () {
        let setweapon = $(this);
        if (setweapon.hasClass('choice_active')) return;
        let weaponindex = localStorage.getItem('weapon_index');
        const idskin = setweapon.attr('id_skin');
        $.ajax({
            type: 'post',
            url: location.href,
            data: { CollectionSkinChangerUpdate: true, collectionID: collectionID, weapon_index: weaponindex, id_skin: idskin},
            dataType: 'json',
            global: false,
            success: function (data) {
                if (typeof noty === 'function') {
                ...
                if (data.status === 'success') {
                    ...
                    interfaceDetailsCollection(collectionID, activeSide, '');
                }
            },
            error: function (error) {
                console.log(error);
    ...
    $(document).on('click', '#CollectionsSkinChangerSettingModal', function () {
        let weaponindex = localStorage.getItem('weapon_index');
        savedObjects = $('#skin-modal-js').children().toArray();
        savedButtons = $('.cnopick').html();
        $('#skin-modal-js').html('<div class="block-loader-modal"><div class="loader-skins-modal"></div></div>')
        $.ajax({
            type: 'post',
            url: location.href,
            data: { CollectionsSkinChangerSettingModal: true, collectionID: collectionID, weapon_index: weaponindex }, 
            dataType: 'json',
            global: false,
            success: function (data) { 
                $('#skin-modal-js').html(data.html);
                $('.cnopick').html('<a class="modal_deff" id="SkinChangerSkins">' + selectel_skins + '</a>');
                renderPaginationCollection(data.count_stickers, 12, 1);
                loaderIMG(3);
            },
            error: function (error) {
                console.log(error);
    ...
    $(document).on('click', '#CollectionsStickerUpdate', function () {
        let setsticker = $(this);
        let savedsticker = setsticker.find('.sticker-modal-img').html();
        let weaponindex = localStorage.getItem('weapon_index');
        const idslot = $('.collections-sk-modal-weapon-sticker.active').attr('id');
        const idsticker = $(this).attr('id_sticker');
        $.ajax({
            type: 'post',
            url: location.href,
            data: { CollectionsStickerUpdate: true, collectionID: collectionID, id_sticker: idsticker, weapon_index: weaponindex, id_slot: idslot },
            dataType: 'json',
            global: false,
            success: function (data) {
                if (typeof noty === 'function') {
                ...
                if (data.status === 'success') {
                    if(idsticker == 0) {
                        ...
                        $('.collections-sk-modal-weapon-sticker.active').html(savedsticker);
                    }
                    interfaceDetailsCollection(collectionID, '');
                }
            },
    ...
    $(document).on('click', '.collections-sk-modal-weapon-sticker', function () {
        const id = $(this).attr('id');
        const id_active = $('.collections-sk-modal-weapon-sticker.active').attr('id');
        ...
        if (idNum === idActiveNum || (idActiveNum >= 1 && idActiveNum <= 4 && idNum !== 5)) {
            ...
            $.ajax({
                type: 'post',
                url: location.href,
                data: { CollectionsStickersKeychainHtml: true, id: idNum },
                dataType: 'json',
                global: false,
                success: function (data) {
                    ...
                    renderPaginationCollection(data.count, 12, 1);
                    loaderIMG(3);
                },
                error: function (error) {
                    console.log(error);
        ...
        $('.collections-sk-modal-weapon-sticker').removeClass('active');
        $(this).addClass('active');
    });



    $(document).on('click', '#CollectionsSkinChangerSetting', function () {
        let weaponindex = localStorage.getItem('weapon_index');
        ...
        $.ajax({
            type: 'post',
            url: location.href,
            data: { CollectionsSkinChangerSetting: true, collectionID: collectionID, weapon_index: weaponindex, stattrack: stattrack, stattrack_count: countstattrak, float: float, pattern: pattern, nametag: nametag }, 
            dataType: 'json',
            global: false,
            success: function (data) { 
                if (typeof noty === 'function') {
    ...
    $(document).on('click', '#CollectionsSkinChangerNoSkin', function () {
        const weaponindex = $(this).attr('weapon_index');
        $.ajax({
            type: 'post',
            url: location.href,
            data: { CollectionsSkinChangerNoSkin: true, collectionID: collectionID, weapon_index: weaponindex }, 
            dataType: 'json',
            global: false,
            success: function (data) {
                if (typeof noty === 'function') {
    ...
    $(document).on('click', '#CollectionsSkinChangerAddKnife', function (e) {
        if ($(e.target).closest('#set-skin-collections').length) return;
        let setweapon = $(this);
        if (setweapon.hasClass('choice_active')) return;
        const id_knife = $(this).attr('id_knife');
        $.ajax({
            type: 'post',
            url: location.href,
            data: { CollectionsSkinChangerAddKnife: true, id_knife: id_knife, collectionID: collectionID }, 
            dataType: 'json',
            global: false,
            success: function (data) {
                if (typeof noty === 'function') {
                ...
                if (data.status === 'success') {
                    $('.block-skin-fon#CollectionsSkinChangerAddKnife').not(setweapon).removeClass('choice_active');
                    setweapon.toggleClass('choice_active');
                    let buttonSkin = setweapon.find('.button-skin');
                    if (!buttonSkin.attr('id')) buttonSkin.text(choose_default);
                }
    ...
    $(document).on('click', '#CollectionsSkinChangerAddGlove', function (e) {
        if ($(e.target).closest('#set-skin-collections').length) return;
        let setweapon = $(this);
        if (setweapon.hasClass('choice_active')) return;
        const id_knife = $(this).attr('id_glove');
        $.ajax({
            type: 'post',
            url: location.href,
            data: { CollectionsSkinChangerAddGlove: true, id_glove: id_knife, collectionID: collectionID }, 
            dataType: 'json',
            global: false,
            success: function (data) {
                if (typeof noty === 'function') {
                ...
                if (data.status === 'success') {
                    $('.block-skin-fon#CollectionsSkinChangerAddGlove').not(setweapon).removeClass('choice_active');
                    setweapon.toggleClass('choice_active');
                    let buttonSkin = setweapon.find('.button-skin');
                    if (!buttonSkin.attr('id')) buttonSkin.text(choose_default);
                }
    ...
    $(document).on('click', '#CollectionCreate', function(e) {
        e.preventDefault();
        $('#popupAddCollection').addClass('visible');
    });

    $(document).on('click', '.modal_collection_btn_cancel', function(e) {
        e.preventDefault();
        $('#popupAddCollection').removeClass('visible');
    });

    let isRequestPending = false; // Флаг для отслеживания состояния запроса

    $(document).on('submit', '#collection-create-form', function(e) {
        e.preventDefault();
    
        if (isRequestPending) {
            noty('Пожалуйста, подождите перед отправкой следующего запроса.', 'warning');
            return;
        }
    
        isRequestPending = true; // Блокируем новые запросы
        ...
        $.ajax({
            ...
            data: {
                ...
                c_name: $('input[name="add_name_collection"]').val()
            },
            dataType: 'json',
            global: false,
            async: true,
            success: function(data) {
                noty(data.message, data.status);
                if (data.status === 'success') {
                    $('input[name="add_name_collection"]').val('');
                    $('#popupAddCollection').removeClass('visible');
    
                    interfaceDetailsCollection(data.id);
    
                    var newUrl = location.protocol + "//" + location.host + location.pathname + data.id + "/";
                    history.pushState({ id: data.id }, '', newUrl);
                }
            },
            complete: function() {
                // Разблокируем отправку новых запросов через 2 секунды
                setTimeout(function() {
    ...
    $(document).on('click', '#CollectionDelete', function(e) {
        e.preventDefault();
        $('#popupDelCollection').addClass('visible');
    });

    $(document).on('click', '.modal_collection_btn_cancel', function(e) {
        e.preventDefault();
        $('#popupDelCollection').removeClass('visible');
    });

    $(document).on('submit', '#collection-del-form', function(e) {
        e.preventDefault();
        $.ajax({
            type: 'POST',
            url: location.href,
            data: { CollectionDelete: true, collectionID: collectionID },
            dataType: 'json',
            global: false,
            async: true,
            success: function(data) {
    ...
    $(document).on('click', '#CollectionPublic', function(e) {
        e.preventDefault();
        $.ajax({
            type: 'POST',
            url: location.href,
            data: { 
                CollectionPublicToggle: true, 
    ...
    $(document).on('click', '#CollectionApply', function(e) {
        e.preventDefault();
        $('#popupApplyCollection').addClass('visible');
    });

    $(document).on('click', '.modal_collection_btn_cancel', function(e) {
        e.preventDefault();
        $('#popupApplyCollection').removeClass('visible');
    });

    $(document).on('submit', '#collection-apply-form', function(e) {
        e.preventDefault();
    ...
    function loadStickers2(textstickers, pagestickers) {
        const id = $('.collections-sk-modal-weapon-sticker.active').attr('id');
        $.ajax({
            type: 'post',
            url: location.href,
            data: { CollectionsSkinChangerStickers: true, text_stickers: textstickers, page_stickers: pagestickers, id: id },
            dataType: 'json',
            global: false,
            success: function (data) {
                const container = $('.stickers-modal-js');
                container.html(data.html);
                renderPaginationCollection(data.count_stickers, 12, pagestickers);
                loaderIMG(3);
            },
            error: function (error) {
                console.log(error);
    ...
    $(document).on('click', '.pagination-btn2', function () {
        const pageNumber = $(this).data('page');
        if (pageNumber) {
            pagestickers = pageNumber;
            loadStickers2($('#search_js_stickers2').val(), pagestickers);
        }
    });

    $(document).on('input', '#search_js_stickers2', function () {
        pagestickers = 1;
        loadStickers2($(this).val(), pagestickers);
    });

    function renderPaginationCollection(totalStickers, stickersPerPage, currentPage) {
        ...
        if (totalPages > 1) {
            ...
            if (startPage > 1) {
                paginationContainer.append('<button class="pagination-btn2" data-page="1">«</button>');
            }
            if (currentPage > 1) {
                paginationContainer.append('<button class="pagination-btn2" data-page="' + (currentPage - 1) + '">‹</button>');
            }
            for (let i = startPage; i <= endPage; i++) {
                paginationContainer.append('<button class="pagination-btn2' + (i === currentPage ? ' active' : '') + '" data-page="' + i + '">' + i + '</button>');
            }
            if (endPage < totalPages) {
                paginationContainer.append('<button class="pagination-btn2" data-page="' + (currentPage + 1) + '">›</button>');
            }
            if (currentPage < totalPages) {
                paginationContainer.append('<button class="pagination-btn2" data-page="' + totalPages + '">»</button>');
            }
        }
    }

    $(document).on('click', '.sk-modal-weapon-sticker2', function () {
        $('.sk-modal-weapon-sticker2').removeClass('active');
        const id = $(this).attr('id');
        $.ajax({
            type: 'post',
            url: location.href,
            data: { CollectionsStickersKeychainHtml: true, id: id },
            dataType: 'json',
            global: false,
            success: function (data) {
                const container = $('.sk-modal-settings-type-2');
